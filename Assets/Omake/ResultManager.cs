...
public class ResultManager : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        var nameList = PlayerPrefsX.GetStringArray("UserNameKEY");
        Debug.Log(timesList);
        ...
        //得点を新しい順に表示するコード//ここに、nameListを組み込む？
        var zip = scoreList.Zip(timesList, (scores, times) => new { scores, times }).Zip(nameList, (s, names) => new { s.scores,s.times,names })
                    .OrderByDescending(pair => pair.times)
                    .Select((pair, index) => $"{pair.names} 得点：{pair.scores} {pair.times}");

                    
       //zip = scoreList.Zip(nameList, (scores, names) => new { scores, names })
       //             .OrderByDescending(pair => pair.names)
       //             .Select((pair, index) => $"{pair.names} 得点：{pair.scores}");
